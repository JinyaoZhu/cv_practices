...
W_forward = trackKLT(I_prev, I, keypoint, r_T, num_iters);
delta_keypoint = W_forward(:,end);
W_backward = trackKLT(I, I_prev, keypoint + delta_keypoint', r_T, num_iters);
delta_keypoint_backward = W_backward(:,end);

keep = norm(delta_keypoint + delta_keypoint_backward) < lambda;
end